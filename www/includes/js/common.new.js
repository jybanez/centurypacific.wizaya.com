...
TPH.Assets = {
	...
	L:['/includes/js/leaflet.js','/includes/css/leaflet.css'],
...
TPH.Ajax = new Class({
	...
	buildRequest:function(options){
		...
		this.req = new Request($merge(options,{
			url:$pick(TPH.$remote,this.options.url),
...
TPH.Implementors = {
	...
	TemplateData:new Class({
		applyTemplateData:function(el,template,data){
			var data = $pick(data,{});
			/*
			var data = $merge($pick(data,{}),{
				$today:new Date().format('%Y-%m-%d'),
				$now:new Date().format('%Y-%m-%d %H:%M:%S')
			});
			*/
			el.set('html',template.substitute(data));
			el.getElements('input[type="text"],input[type="number"],input[type="date"],input[type="time"]').each(function(el){
			...
			el.getElements('.propertyContent').each(function(el){
				el.set(el.get('data-property'),el.get('data-content')).removeProperties('data-property','data-content').removeClass('propertyContent');
			});
			el.getElements('input[type="checkbox"]').each(function(el){
				var value = el.get('data-value');
				if ($defined(value)) {
					el.set('checked',value.split(',').contains(el.get('value')));	
				}	
			});
			el.getElements('input[type="radio"]').each(function(el){
				if ($defined(el.get('data-value'))) {
					el.set('checked',el.get('data-value').split(',').contains(el.get('value')));	
				}	
			});
			el.getElements('.valueCheck').each(function(el){
				var field = el.get('data-field');
				if (!$pick(data[field],'').length) {
					el.destroy();
				}	
				el.removeProperty('data-field').removeClass('valueCheck');
			});
			el.getElements('.noValueCheck').each(function(el){
				var field = el.get('data-field');
				if ($pick(data[field],'').length) {
					el.destroy();
				}	
				el.removeProperty('data-field').removeClass('noValueCheck');
			});
			el.getElements('.inValue').each(function(el){
			...
			el.getElements('.notInValue').each(function(el){
				var separator = $pick(el.get('data-separator'),',');
				var value = el.get('data-value');
				var field = el.get('data-field');
				var list = $type(data[field])=='array'?data[field]:$pick(data[field],'').split(separator);
			...
			el.getElements('.hasValue').each(function(el){
				var separator = $pick(el.get('data-separator'),',');
				var value = el.get('data-value').split(separator);
				var field = el.get('data-field');
				if (!value.contains(data[field])) {
					el.destroy();
				}
			});
			el.getElements('.inputElement').each(function(el){
				var defaults = {},
					properties = {};
				...
				TPH.createInput(el.get('data-type'),{
					name:el.get('data-name'),
					value:el.get('data-value'),
					defaults:defaults,
					required:el.get('data-required')=='1',
...
$extend(TPH.Implementors,{
	Templates:new Class({
		...
		dynamicTemplate:function(name,status){
			...
			if ($defined(status)) {
				...
				if (status){
					...
					template.containment.setProperty('class','fullHeight');
				}
			}
		...
		createTemplate:function(name,container,fullHeight){
			...
			//containment.addClass(parent.hasClass('fullHeight')?'fullHeight':'overflow hidden position relative');
			...
			containment.addEvents({
				mousedown:function(e){
					...
					if (!template.scrolling) {
						template.scroller.start();	
						template.scrolling = true;
					}
				}.bind(this),
				mouseup:function(e){
					console.log(e);
					if (template.scrolling) {
						template.scroller.stop();
						template.scrolling = false;	
					}
			...
			*/
			...
			template.hammer = new Hammer.Manager(template.containment,{
				preventDefault:true,
				domEvents:true
			});
			var	direction = Hammer.DIRECTION_ALL;
	    	template.hammer.add(new Hammer.Pan({ direction:direction, threshold:20, pointers:0 }));
	    	//template.hammer.add(new Hammer.Swipe()).recognizeWith(template.hammer.get('pan'));
	    	
	    	template.hammer.on('swipe',function(e){
	    		//console.log('swipe');
	    		e.preventDefault();
	    		return;
	    		if ($defined(template.panStart)) {
	    			var velocity = {
			    	...
			    	template.slide
						...
						.setDelta(delta)
						.start(template.panStart);	
	    		}
				
	    	}.bind(this));
	    	
	    	template.hammer.on('panstart', function(e) {
	    		//console.log(e);
	    		template.panStart = null;
	    		if (!e.target.hasClass('noTouch')) {
	    			template.slide.cancel();
					template.panStart = template.parent.getScroll();	
	    		} 
			}.bind(this));
	    	
			template.hammer.on('panmove panend', function(e) {
				//console.log('panmove');
				if ($defined(template.panStart)) {
					var x = template.panStart.x-e.deltaX,
						y = template.panStart.y-e.deltaY;		
				    template.parent.scrollTo(x,y);		
				}
			}.bind(this));
			
			template.hammer.on('panend', function(e) {
				//console.log('panend');
				template.panStart = null;
			}.bind(this));	
			
			template.slide = new TPH.Slide(template.parent);
			*/
			return this;
		},
		updateTemplate:function(name){
			...
			if ($defined(template)) {
				...
				if ($defined(template.dynamic)) {
					if (template.dynamic) {
						//if ($defined(template.itemHeight)) {
						//	template.totalHeight =0;
						//	template.items.each(function(item,i){
						//		template.items[i].height = $pick(item.height,template.itemHeight);
						//		template.totalHeight += item.height;
						//	});
							template.containment.setStyles({
		...
		applyTemplate:function(name,data,onCreateElement){
			...
			/*
			var canRender = $type(template.filterFunction)=='function'?template.filterFunction(data):true;
			
			if (canRender) {
				var el = this.createTemplateElement(template,data).inject(template.container);
				if ($type(onCreateElement)=='function') {
					onCreateElement(el,template,data);
				}
				...
				template.totalHeight += coords.height;
				this.updateTemplate(name);	
				
				if (coords.bottom>template.parent.getCoordinates().height) {
					el.remove();
				}
			}
			*/
		...
		renderTemplate:function(name,onRender){
			...
			if (template.dynamic) {
				...
				for (var i=0;i<count;i++) {
					...
					//if (position>scroll.y+parentHeight+height) {
					//	break;
					//}
				}
		...
		updateTemplateElement:function(template,el,data){
			...
			return this.applyTemplateData(el,template.template,data);
		},
...
TPH.storageType = new Class({
	initialize:function(id){
		this.id = id;
	},
	set:function(data,onSet){},
	get:function(onGet){}
});

TPH.storageTypes = {
	localStorage:new Class({
		Extends:TPH.storageType,
		set:function(data,onSet){
		...
		get:function(onGet){
			if ($type(onGet)=='function') {
				onGet(localStorage.getItem(this.id));
			}
		}
	}),
	fileSystem:new Class({
		Extends:TPH.storageType,
		set:function(data,onSet){
			var quota = data.length;
			navigator.webkitPersistentStorage.requestQuota(quota,function(){
				window.webkitRequestFileSystem(window.PERSISTENT , quota, function(storage){
					storage.root.getFile(this.id,{create:true},function(fileEntry){
						fileEntry.createWriter(function(content){
		...
		get:function(){
			navigator.webkitPersistentStorage.requestQuota(quota,function(){
				window.webkitRequestFileSystem(window.PERSISTENT , quota, function(storage){
					storage.root.getFile(this.id,{create:false},function(fileEntry){
...
TPH.localStorage = new Class({
	...
	initialize:function(id,options){
		...
		if ($defined(window.localStorage)) {
			localStorage.setItem('_TPHTEST_', '$TPH$');
			if (localStorage.getItem('_TPHTEST_') != null){
				this.$localStorage = true;
			    localStorage.removeItem('_TPHTEST_');
			}	
		}
		this.$filesystem = false;
		if ($defined(window.webkitRequestFileSystem)) {
	...
	getEngine:function(){
		if (this.$filesystem) {
			return new TPH.storageTypes.fileSystem(this.id);
		} else if (this.$localStorage) {
			return new TPH.storageTypes.localStorage(this.id);
		}
	},
	setData:function(data,onSet){
		this.$engine.set(this.compress(Json.encode(data)),onSet);
	},
	getData:function(onGet){
		this.$engine.get(function(data){
			if ($type(onGet)=='function') {
				var tdata = {};
				try {
					tdata = Json.decode(this.decompress(data));
				} catch(e){
					console.log(e);
				}
				onGet($pick(tdata,{}));
	...
	getStorage:function(){
		...
		//	localStorage.setItem(this.id,this.compress(Json.encode({})));
		//}
		try{
			try {
				var jsonstring = this.decompress(localStorage.getItem(this.id));	
			} catch(e) {
				console.log(e);
			}
			var storage = Json.decode(jsonstring);	
		} catch(e) {
			console.log(e);
		}
		 
		return $defined(storage)?storage:{};
	},
	*/
	set:function(key,value,onSet){
		this.getData(function(storage){
			storage[key] = value;
			this.setData(storage,onSet);
		}.bind(this));
					
		/*
		var storage = this.getStorage();
		storage[key] = value;
		localStorage.setItem(this.id,this.compress(Json.encode(storage)));
		*/
	},
	...
	//},
	erase:function(key,onErase){
		this.getData(function(storage){
			delete storage[key];
			this.setData(storage,onErase);
		}.bind(this));
		/*
		var storage = this.getStorage();
		delete storage[key];
...
TPH.SelectList = new Class({
	...
	initialize:function(options){
		...
		this.applyTemplateData(this.container,this.options.templates.content,this.options.data);
...
TPH.Dropdown = new Class({
	...
	position:function(dd){
		...
		var coords = dd.getCoordinates(window.document.body);
		var height = coords.height-(coords.bottom-winSize.y)-26;
...
TPH.Component = new Class({
	...
	view:function(id,forceLoad,onView){
		...
		if ($defined(data)) {
			this.fireEvent('onBeforeLoadDetails',[data]);
	...
	save:function(){
		...
		if ($defined(form)?TPH.validateForm(form):true) {
			var data = template.container.toQueryString().parseQueryString();
			...
			if (this.validate(data)) {
				...
				this.saveRoutine(data,function(result){
					...
					this.fireEvent('onSave',[this.getData(result.data[this.options.viewKey])]);
					...
					if ($defined(this.options.templates.details) && this.options.autoDetails) {
						el.addEvent('click',function(e){
							if ($defined(this.options.listItemFunction)) {
								this.options.listItemFunction(el,item);
							} else {
	...
	liveUpdate:function(limitstart){
		...
		this.$liveUpdateRequest = new TPH.Json({
			...
			onFailure:function(){
				this.$liveUpdate = this.liveUpdate.delay(500,this,limitstart);
			}.bind(this)
		}).request();
		return this;
	},
...
TPH.Map = new Class({
	...
	showUserLocation:function(){
		if ($defined(TPH.$gps)) {
			var position = L.latLng(TPH.$gps.coords.latitude,TPH.$gps.coords.longitude);
	...
	addMarker:function(lat,lng,data,reCenter) {
		...
		if (!$defined(marker)) {
			var marker = L.marker(position,this.options.marker).bindTooltip(this.options.template.substitute(data)).addTo(this.mapInstance);
			this.fireEvent('onMarkerCreate',[marker,data,this]);
			marker.on('click',function(e){
				this.fireEvent('onMarkerClick',[marker,data,this]);
			}.bind(this));
...
function $fullHeight(container){	
	...
	var paddings = container.getStyle('padding-top').toInt()+container.getStyle('padding-bottom').toInt();
	var borders = container.getStyle('border-top-width').toInt()+container.getStyle('border-bottom-width').toInt();
	var offset = paddings+borders;
	var els = new Array();
		...
		if (!['absolute','fixed'].contains(el.getStyle('position'))) {
			if (!['svg'].contains(el.get('tag'))) {
				...
				if (el.hasClass('fullHeight') && !isAutoHeight && display!='none'){ //el.getStyle('height')!='auto') {
					...
					offset+=el.getStyle('margin-top').toInt()+el.getStyle('margin-bottom').toInt()+el.getCoordinates().height+el.getStyle('border-top-width').toInt()+el.getStyle('border-bottom-width').toInt();
				}	
			} else {
				el.setStyle('display','');
			}	
	...
	els.each(function(el){		
		...
		var theight = container.clientHeight-paddings; //-borders; //-el.getStyle('margin-top').toInt()-el.getStyle('margin-bottom').toInt()-el.getStyle('border-top-width').toInt()-el.getStyle('border-bottom-width').toInt();
