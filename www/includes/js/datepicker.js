...
var DatePicker = new Class({
	...
	onFocus: function(original_input, visual_input) {
		...
		if ($chk(original_input.get('value'))) {
			...
			if ($chk(this.options.maxDate) && init_visual_date.valueOf() > this.maxDate.valueOf()) {
				init_visual_date = new Date(this.maxDate.valueOf());
			}
			if ($chk(this.options.minDate) && init_visual_date.valueOf() < this.minDate.valueOf()) {
				init_visual_date = new Date(this.minDate.valueOf());
			}
		}
	...
	renderTime: function() {
		...
		var hours = this.d.getHours();
		new Element('input', { type: 'text', 'class': 'hour' })
			.set('value', this.leadZero(hours>12?hours-12:hours))
			.addEvents({
		...
		new Element('input', { type: 'submit', value: 'OK', 'class': 'ok' })
			.addEvents({
				click: function(e) {
					...
					this.select($merge(this.dateToObject(this.d), { 
						hours: picker.getElement('.hour').get('value').toInt()+(picker.getElement('.ampm').get('value')=='am'?0:12), 
						minutes: picker.getElement('.minutes').get('value').toInt() 
					}));
				}.bind(this)
			})
			.set('maxlength', 2)
		...
		new Element('select',{'class':'ampm'}).inject(container)
			.adopt(new Element('option',{value:'am'}).set('html','AM'))
			.adopt(new Element('option',{value:'pm'}).set('html','PM'))
			.set('value',hours>12?'pm':'am')
			;
	},
	...
	renderDecades: function() {
		...
		if ($chk(this.options.minDate) && this.d.getFullYear() <= this.minDate.getFullYear()) {
			this.limit.left = true;
		}
	...
	select: function(values) {
		...
		this.fireEvent('onSelect',[d,this]);
