(function(){
	...
	TPH.CSS = {
		Rule:new Class({
			...
			initialize:function(property,value,options){
				this.$property = property;
				this.$value = value;
			},
			setValue:function(value,refresh){
				this.$value = value;
				if ($pick(refresh,true)) {
					this.fireEvent('onChange',[this]);	
				}
				return this;
			},
			render:function(prefix){
				var prefix = $pick(prefix,'');
				return prefix+this.$property+':'+this.$value+';';
			},
			toObject:function(){
				return {
					type:'rule',
		...
		Declaration:new Class({
			Implements:[Events,Options],
			initialize:function(selector,rules,options){
				this.setOptions(options);
				this.$selector = selector;
				this.$rules = new Array();
				rules.each(function(rule){
					this.setRule(rule.property,rule.value,rule.options,false);
				}.bind(this));
			},
			...
			getRule:function(property){
				var count = this.$rules.length;
				for(var i=0;i<count;i++) {
					if (this.$rules[i].$property==property){
						return this.$rules[i];
					}
				}
			},
			setRule:function(property,value,options,refresh){
				var trule = this.getRule(property);
				if ($defined(trule)) {
					trule.setValue(value,false);	
				} else {
					var trule = new TPH.CSS.Rule(property,value,options);
					trule.addEvents({
			...
			removeRule:function(property){
				var trule = this.getRule(property);
				if ($defined(trule)) {
					this.$rules.erase(trule);
					trule.destroy();
					this.fireEvent('onChange',[this]);	
				}
				return this;
			...
			render:function(prefix){
				...
				this.$rules.each(function(rule){
					rules.push(prefix+rule.render("\t"));
				}.bind(this));
				rules.push(prefix+"}");
				return prefix+this.$selector+rules.join("\n");
			},
			toObject:function(){
				var rules = new Array();
				this.$rules.each(function(rule){
					rules.push(rule.toObject());
				}.bind(this));
				return {
					type:'declaration',
		...
		Block:new Class({
			Implements:[Events,Options],
			initialize:function(label,declarations,options){
				this.setOptions(options);
				this.$label = label;
				this.$declarations = new Array();
				declarations.each(function(declaration){
					var tdeclaration = new TPH.CSS.Declaration(declaration.selector,declaration.rules,declaration.options);
					tdeclaration.addEvents({
						change:function(){
							this.fireEvent('onChange',[this]);
						}.bind(this)
					});
					this.$declarations.push(tdeclaration);	
				}.bind(this)); 
			},
			setLabel:function(label){
				this.$label = label;
				this.fireEvent('onChangeLabel',[this]);
			},
			addDeclaration:function(selector,rules,options){
				var tdeclaration = new TPH.CSS.Declaration(selector,rules,options);
				tdeclaration.addEvents({
					change:function(){
						this.fireEvent('onChange',[this]);
					}.bind(this)
			...
			removeDeclaration:function(declaration){
				this.$declarations.erase(declaration);
				declaration.destroy();
				this.fireEvent('onChange',[this]);
				return this;
			},
			render:function(prefix){
				...
				this.$declarations.each(function(declaration){
					declarations.push(prefix+declaration.render(prefix+"\t"));
				}.bind(this));
				declarations.push(prefix+"}");
				return prefix+this.$label+declarations.join("\n");
			},
			toObject:function(){
				var declarations = new Array();
				this.$declarations.each(function(declaration){
					declarations.push(declaration.toObject());
				}.bind(this));
				return {
					type:'block',
					label:this.$label,
	...
	TPH.Styles = new Class({
		...
		initialize:function(styles,options){
			...
			styles.each(function(style){
				switch(style.type){
					case 'declaration':
						var declaration = new TPH.CSS.Declaration(style.selector,style.rules,style.options);
						this.addDeclaration(declaration,false);
						break;
					case 'block':
						var block = new TPH.CSS.Block(style.label,style.declarations,style.options);
		...
		addDeclaration:function(declaration,refresh){
			var refresh = $pick(refresh,true);
			declaration.addEvents({
				change:function(){
					this.refresh();
				}.bind(this)
			});
			this.$styles.push(declaration);
			return refresh?this.refresh():this;
		},
		addBlock:function(block,refresh){
			var refresh = $pick(refresh,true);
			block.addEvents({
				change:function(){
					this.refresh();	
		...
		removeStyle:function(style){
			this.$styles.erase(style);
			this.refresh();
			return this;
		},
		render:function(container){			
			if (!$defined(this.$element)) {
				this.$element = new Element('style');
			}
		...
		refresh:function(){
			if ($defined(this.$element)){
				...
				this.$styles.each(function(style){
					styles.push(style.render());
				}.bind(this));
				this.$element.empty().set('text',styles.join("\n"));
			}
			return this;
		},
		toObject:function(){
